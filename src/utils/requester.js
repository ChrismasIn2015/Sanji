...
export class SanjiRequester {
    ...
    request(methods, url, params, config) {
        return new Promise((resolve, reject) => {
            // 验证用户设置
            if (!this.BASE_URL) reject('您未初始化 SanjiRequester 无法请求!')
            if (!this.COMPLETE)
                reject('您未设置 SanjiRequester 的 success回调函数！')
            if (!this.ERROR)
                reject('您未设置 SanjiRequester 的 error回调函数！')
