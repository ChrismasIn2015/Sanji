<template>
    <div class="stock flex-y">
        ...
        <div class="stock-nav flex">
            <div class="common-btn" @click.stop="updateMyStockStore">更新</div>
        </div>
        <div class="stock-nav flex">
            <div class="common-btn toggle" @click.stop="pageIndex = 0">
                返回
            </div>
        </div>
        ...
        <div class="stock-content">
            ...
            <div class="stock-table flex" v-if="pageIndex === 1">
                <div v-for="(item, index) in blockMapKeyList" :key="index">
                    <div class="column" @click.stop="setPageIndex(1, item)">
                        {{ item.typeName }}
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            setPageIndex(oldIndex, item) {
                if (oldIndex === 0) {
                    this.pageIndex = 1
                    this.blockMapKeyList = Object.assign(
                        [],
                        this.blockMap[item]
                    )
                }
